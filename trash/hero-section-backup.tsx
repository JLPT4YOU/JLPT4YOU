...
// Lazy load animation with intersection observer for better performance
const SimpleTetrisAnimation = dynamic(() => import("./simple-tetris-animation").then(mod => ({ default: mod.SimpleTetrisAnimation })), {
  ssr: false,
  loading: () => <div className="w-full h-full bg-gradient-to-br from-primary/10 to-secondary/10 rounded-lg animate-pulse" />
})

// Star type definition - DISABLED for performance
/*
interface Star {
  id: string
  x: number
...
// Sparkling Stars Background Component - DISABLED for performance
const SparklingStarsBackground = () => {
  ...
  useEffect(() => {
    ...
    // Large bright stars first (need most space) - further reduced for performance
    for (let i = 0; i < 2; i++) {
    ...
    // Medium stars (less common) - further reduced for performance
    for (let i = 0; i < 5; i++) {
      const position = generateSafePosition(stars)
      stars.push({
        id: `medium-${i}`,
    ...
    // Small twinkling stars (most common) - significantly reduced for performance
    for (let i = 0; i < 12; i++) { // Significantly reduced for better performance
      const position = generateSafePosition(stars)
      stars.push({
        id: `small-${i}`,
        x: position.x,
...
export function HeroSection({ translations }: HeroSectionProps) {
  ...
  return (
    <section className="relative py-12 md:py-20 lg:py-24 bg-background overflow-hidden min-h-[600px] md:min-h-[700px] lg:min-h-[800px]">
      {/* Simple gradient background instead of animated stars for better performance */}
