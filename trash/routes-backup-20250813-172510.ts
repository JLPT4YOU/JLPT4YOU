...
// Route feature types
export type RouteFeature = 'home' | 'jlpt' | 'challenge' | 'driving' | 'dict' | 'study' | 'library' | 'auth'
...
export const routes = {
  ...
  study: (language: Language) =>
    generateRoute('study', language, {}),

  studyTheory: (language: Language) =>
    generateRoute('study', language, { type: 'theory' }),

  studyPractice: (language: Language) =>
    generateRoute('study', language, { type: 'practice' }),
...
export function isValidRoute(pathname: string): boolean {
  ...
  const validFeatures: RouteFeature[] = ['home', 'jlpt', 'challenge', 'driving', 'dict', 'auth']
