...
function LanguageProviderInner({
  ...
  const currentLanguage = React.useMemo(() => {
    ...
    if (!hasLangPrefix) {
      ...
      console.log(`[LanguageContext] Clean URL detected: ${pathname}, stored language: ${storedLanguage}, path language: ${pathLanguage}, isAuthenticated: ${isAuthenticated}`)
      if (storedLanguage) {
        return storedLanguage
      }
    }

    console.log(`[LanguageContext] Language detection: pathname=${pathname}, isAuthenticated=${isAuthenticated}, pathLanguage=${pathLanguage}`)
    return pathLanguage
  ...
  useEffect(() => {
    ...
    if (!hasLangPrefix) {
      ...
      const storedLanguage = getStoredLanguagePreference()
      console.log(`[LanguageContext] Clean URL effect: pathname=${pathname}, isAuthenticated=${isAuthenticated}, currentLanguage=${currentLanguage}, storedLanguage=${storedLanguage}`)

      if (storedLanguage && storedLanguage !== currentLanguage) {
        console.log(`[LanguageContext] Reloading translations for stored language: ${storedLanguage}`)
        setForceRefresh(prev => prev + 1)
      }
    }
