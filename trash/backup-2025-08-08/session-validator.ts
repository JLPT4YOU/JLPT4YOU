...
export class SessionValidator {
   ...
   * Maps custom cookie name 'jlpt4you-auth' to Supabase's expected format
   */
  private static createSupabaseClient(request: NextRequest) {
    // Get the custom cookie value
    const customCookie = request.cookies.get('jlpt4you-auth')
    ...
    return createServerClient(
      ...
      process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!,
      {
        cookies: {
          get(name: string) {
            // Supabase expects cookies with specific names based on the project URL
            // Map our custom cookie to what Supabase expects
            if (name.includes('-auth-token') && customCookie?.value) {
              // The custom cookie value is already in the correct format
              return customCookie.value
            }
            // Fallback to checking the actual cookie name
            return request.cookies.get(name)?.value
          },
          set() {
            // No-op for validation - we don't want to modify cookies during validation
          },
          remove() {
