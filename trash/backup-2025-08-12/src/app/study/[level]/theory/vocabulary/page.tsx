// BACKUP 2025-08-12 of src/app/study/[level]/theory/vocabulary/page.tsx
"use client"
...
function Content() {
  ...
  const fetchSearchWords = async (reset = false) => {
    ...
    try {
      const currentOffset = (reset ? 0 : page * limit)
      const data = await searchWords(query.trim(), { level, limit, offset: currentOffset })
    ...
    <div className="min-h-screen bg-background">
      <div className="app-container py-6">
        <div className="app-content max-w-6xl mx-auto">
          ...
          />
          <WordGrid words={words} loading={loading} error={error} />
          <FooterControls
            loading={loading}
            hasNext={hasNext}
            page={page}
...
function Header({ level }: { level: string }) {
  return (
    <div className="text-center app-space-md">
      <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold tracking-tight">Study {level.toUpperCase()} - Vocabulary</h1>
      <p className="text-muted-foreground">Learn JLPT vocabulary by level. Use search to find specific words.</p>
    </div>
  )
}
    ...
    <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 mb-6">
      <div className="flex-1 flex items-center gap-2">
        <input
          ...
          placeholder="Search word (kanji/hiragana/meaning)"
...
function WordGrid({ words, loading, error }: { words: JLPTWord[]; loading: boolean; error: string | null }) {
  if (error) {
    return (
  ...
  return (
    <>
      {words.length === 0 && !loading ? (
        <div className="text-center text-muted-foreground">No words found</div>
      ) : (
