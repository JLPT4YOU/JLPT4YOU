...
export class GeminiService extends BaseAIService {
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        while (getFile.state === 'PROCESSING') {
          ...
          await new Promise((resolve) => {
            setTimeout(resolve, 2000); // Check every 2 seconds
          });
        }
   ...
   * Note: Embedding model has been removed. Use text-embedding-004 or similar if needed.
   */
  async generateEmbedding(_text: string): Promise<number[]> {
    console.warn('Embedding model has been removed from configuration');
    throw new Error('Embedding functionality is no longer available. Please use alternative embedding services.');
  }
...
export const formatMessagesForGemini = (messages: AIMessage[]): GeminiMessage[] => {
  return messages
    .filter(msg => msg.role !== 'system') // Filter out system messages
    .map(msg => createGeminiMessage(msg.content, msg.role === 'assistant' ? 'model' : 'user'));
};
