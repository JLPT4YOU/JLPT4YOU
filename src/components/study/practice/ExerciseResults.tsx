              ...
              {session.answers.map((answer, index) => (
                <div
                  ...
                  className={`
                    ...
                    ${answer.isCorrect
                      ? 'bg-green-50 text-green-700 border border-green-200 dark:bg-green-950/20 dark:text-green-400 dark:border-green-400/30'
                      : 'bg-red-50 text-red-700 border border-red-200 dark:bg-red-950/20 dark:text-red-400 dark:border-red-400/30'
                    }
                  `}
                  ...
                  <Card key={q.id} className="border-muted">
                    <CardContent className="p-4 space-y-2">
                      <div className="flex items-start justify-between">
                        ...
                        <div className={`text-xs px-2 py-0.5 rounded ${isCorrect ? 'bg-green-50 text-green-700 dark:bg-green-950/20 dark:text-green-400' : 'bg-red-50 text-red-700 dark:bg-red-950/20 dark:text-red-400'}`}>
                          {isCorrect ? t('study.practice.exercise.correct') : t('study.practice.exercise.incorrect')}
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                        {q.options?.map((opt: string, i: number) => {
                          ...
                          return (
                            <div key={i} className={`text-sm p-2 rounded border ${isAns ? 'border-green-300 bg-green-50 dark:border-green-400/30 dark:bg-green-950/20' : isUser ? 'border-red-300 bg-red-50 dark:border-red-400/30 dark:bg-red-950/20' : 'border-muted'}`}>
                              <div className="flex items-center gap-2">
                                <span className="font-mono text-xs">{String.fromCharCode(65 + i)}.</span>
                                <span>{opt}</span>
                              </div>
                              {isAns && <div className="text-xs text-green-700 dark:text-green-400 mt-1">{t('study.practice.results.correctAnswer')}</div>}
                              {isUser && !isAns && <div className="text-xs text-red-700 dark:text-red-400 mt-1">{t('study.practice.results.yourAnswer')}</div>}
                            </div>
                          )
                        })}
                      </div>
                      ...
                      <div className="mt-2 text-sm">
                        <div className="font-medium mb-1">{t('study.practice.results.explanation')}</div>
                        {hasDetailed ? (
                          <div className="space-y-1">
                            <div><span className="text-muted-foreground">{t('study.practice.results.whyCorrect')}</span> {(q.explanation as any)?.why_correct}</div>
                            {Boolean((q.explanation as any)?.wrong_answers) && (
                              <div className="text-muted-foreground">{t('study.practice.results.whyWrong')}</div>
                            )}
                            ...
                            {Boolean((q.explanation as any)?.notes) && (
                              <div><span className="text-muted-foreground">{t('study.practice.results.notes')}</span> {(q.explanation as any).notes}</div>
                            )}
                          </div>
                        ) : (
