...
const ShikiCodeBlock: React.FC<ShikiCodeBlockProps> = ({
  ...
  useEffect(() => {
    ...
    const highlightCode = async () => {
      ...
      try {
        ...
        const currentTheme = theme === 'dark' ? 'dracula-soft' : 'github-light-default'
    ...
    return (
      <code
        className={cn(
          ...
          // Allow wrapping for long inline code to avoid horizontal scrolling on mobile
          "whitespace-pre-wrap break-words max-w-full inline-block align-middle"
        )}
  ...
  // Block code (non-inline): progressive render to avoid flicker
  // Always render a plain version first, then fade-in Shiki when ready
  if (!shouldRenderInline) {
    ...
    const showHighlighted = !!highlightedHtml && !isStreaming
    ...
    return (
      <div className="relative group my-4 w-full max-w-full">
        ...
        {/* Plain code (visible during streaming or until highlight ready) */}
        <pre
          className={cn(
        ...
        {/* Shiki highlighted code (fades in when ready) */}
        {showHighlighted && (
          <div
            className={cn(
              "absolute inset-0 shiki-code-block overflow-x-auto overflow-y-hidden",
              "[&_pre]:!m-0 [&_pre]:!p-0",
            ...
            style={{ ...containerStyle, willChange: 'opacity' }}
