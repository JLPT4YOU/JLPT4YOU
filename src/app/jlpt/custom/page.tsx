...
function JLPTCustomPageContent({ t }: JLPTCustomPageContentProps) {
  const jlptLevels = [
    {
      level: "N1",
              ...
              {jlptLevels.map((level) => (
                <Link key={level.level} href={level.href}>
                  <div className="group cursor-pointer bg-muted/10 rounded-2xl p-3 sm:p-4 md:p-6 text-center transition-all duration-200 hover:bg-muted/30 border border-border/20">
                    <div className={`w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 mx-auto rounded-full flex items-center justify-center mb-2 sm:mb-3 md:mb-4 group-hover:scale-105 transition-transform duration-200 ${level.bgColor} relative`}>
