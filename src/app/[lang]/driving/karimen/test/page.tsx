...
interface KarimenTestPageProps {
  params: Promise<{
    lang: string
  }>
}

function KarimenTestContent({ params }: KarimenTestPageProps) {
  ...
  const getTimeLimit = (): number => {
    ...
    return 30 // Default 30 minutes for Karimen
  }

  const getExamTitle = (): string => {
    return 'Driving License Test - Karimen'
  }
  ...
  const handleExamSubmit = (answers: Record<number, 'A' | 'B' | 'C' | 'D'>) => {
    console.log('Karimen exam submitted:', answers)
    // Navigate to results page
    const params = new URLSearchParams({
      type: 'driving',
      level: 'karimen',
      sections: 'basic-rules,signs',
      demo: 'driving-karimen-good'
    })
    if (resolvedParams) {
  ...
  const handleExamPause = () => {
    ...
    if (resolvedParams) {
      router.push(`/${resolvedParams.lang}/driving/karimen`)
    }
  }
...
export default function KarimenTestPage({ params }: KarimenTestPageProps) {
  return (
    <Suspense fallback={<div>Loading...</div>}>
      <KarimenTestContent params={params} />
    </Suspense>
  )
}
