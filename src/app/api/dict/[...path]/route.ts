...
function getJDictConfig() {
  const apiBase = process.env.JDICT_API_BASE;
  const staticBase = process.env.JDICT_STATIC_BASE;

  if (!apiBase || !staticBase) {
    throw new Error('JDICT_API_BASE and JDICT_STATIC_BASE environment variables must be set');
  }
...
export async function GET(
  ...
  try {
    const { apiBase, staticBase } = getJDictConfig();
    ...
    if (path === 'radicals') {
      ...
      targetUrl = `${staticBase}/radical.json`;
