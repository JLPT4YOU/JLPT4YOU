...
export default function StudyHistoryByLevelPage() {
  return (
    <ProtectedRoute>
      <LanguagePageWrapper>
          ...
          <div className="max-w-4xl mx-auto app-space-2xl">
            ...
            <header className="text-center app-space-lg">
              ...
              <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold tracking-tight text-foreground">
                {t('study.history.levelTitle').replace('{level}', level.toUpperCase())}
              </h1>
              <p className="text-lg sm:text-xl text-muted-foreground mt-4">
                {t('study.history.selectType')}
              </p>
            </header>
            ...
            <main className="flex justify-center app-space-xl" role="main">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 md:gap-8 place-items-center w-max">
                {/* Vocabulary History Card */}
                <Link
                  href={`/study/${level}/history/vocabulary`}
                  ...
                  aria-label={`${t('study.vocabulary.title')} ${t('study.history.title')} ${level.toUpperCase()}`}
                >
                  <article className="bg-muted/10 rounded-2xl p-6 md:p-8 text-center transition-all duration-200 hover:bg-muted/30 border border-border/20 group-hover:scale-105">
                    ...
                    <p className="text-muted-foreground text-sm">{t('study.history.vocabulary.description')}</p>
                  </article>
                </Link>

                {/* Grammar History Card */}
                <Link
                  href={`/study/${level}/history/grammar`}
                  className="block w-full max-w-[320px] group focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 rounded-2xl"
                  aria-label={`${t('study.grammar.title')} ${t('study.history.title')} ${level.toUpperCase()}`}
                >
                  <article className="bg-muted/10 rounded-2xl p-6 md:p-8 text-center transition-all duration-200 hover:bg-muted/30 border border-border/20 group-hover:scale-105">
                    <div className="w-16 h-16 mx-auto rounded-full flex items-center justify-center mb-4 group-hover:scale-105 transition-transform duration-200 bg-primary/10 group-hover:bg-primary/20">
                    ...
                    <p className="text-muted-foreground text-sm">{t('study.history.grammar.description')}</p>
                  </article>
                </Link>

                {/* Reading History Card */}
                <Link
                  href={`/study/${level}/history/reading`}
                  className="block w-full max-w-[320px] group focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 rounded-2xl"
                  aria-label={`${t('study.practice.types.reading')} ${t('study.history.title')} ${level.toUpperCase()}`}
                >
                  <article className="bg-muted/10 rounded-2xl p-6 md:p-8 text-center transition-all duration-200 hover:bg-muted/30 border border-border/20 group-hover:scale-105">
                    <div className="w-16 h-16 mx-auto rounded-full flex items-center justify-center mb-4 group-hover:scale-105 transition-transform duration-200 bg-primary/10 group-hover:bg-primary/20">
                    ...
                    <p className="text-muted-foreground text-sm">{t('study.history.reading.description')}</p>
                  </article>
                </Link>
              </div>
            </main>
