...
import { safeGetJSON, safeSetJSON, safeRemoveItem } from '../shared/storage-utils'
...
export class LocalStorageService {
  ...
  /**
   * Generic storage methods (delegated to shared utils)
   */
  private getItem<T>(key: string): T | null {
    return safeGetJSON<T>(key)
  }

  private setItem<T>(key: string, value: T): boolean {
    return safeSetJSON<T>(key, value)
  }
