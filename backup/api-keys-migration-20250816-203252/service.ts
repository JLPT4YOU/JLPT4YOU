...
const DEFAULT_API_KEY = "WBBcwnwQpV89"; // public per docs

export function buildDjUrl(value: string, apiKey: string = DEFAULT_API_KEY): string {
  // Use local proxy to bypass CORS in browser. The proxy forwards to upstream.
...
export async function fetchDj(value: string, opts?: { apiKey?: string; timeoutMs?: number }): Promise<DjResult> {
  const url = buildDjUrl(value, opts?.apiKey);
