...
export function ExerciseGenerator({
  ...
  const shouldDisableThinkingToggle = isProModel;

  const [selectionMode, setSelectionMode] = useState<'random' | 'sequential'>('random');
  const [lesson, setLesson] = useState(1);
  const [materialLimit, setMaterialLimit] = useState<number>(type === 'grammar' ? 3 : 5);
  ...
  const handleGenerate = () => {
    const materialLimitVal = selectionMode === 'random'
      ? (type === 'grammar' ? 10 : 50)
      : materialLimit;
    onGenerate({
      count: questionCount,
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        ...
        {/* Move Model + Thinking to bottom per request: keeping header clean */}
        ...
        <CardContent className="space-y-4">
            ...
            <div className="space-y-3 p-3 bg-green-50 rounded-lg border border-green-200">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <p className="text-xs text-green-700 mt-1">{t('study.practice.generator.lesson')} {lesson} (offset: {startOffset})</p>
                </div>
                <div>
                  ...
                  <p className="text-xs text-green-700 mt-1">{type === 'grammar' ? t('study.practice.generator.sequentialOptions.defaultGrammar') : t('study.practice.generator.sequentialOptions.defaultVocabulary')}</p>
                </div>
              </div>
            </div>
          )}
          ...
          <div className="rounded-lg bg-muted p-3">
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 text-sm">
              ...
              <div className="text-center">
                <div className="text-muted-foreground text-xs">{t('study.practice.generator.summary.questions')}</div>
              ...
              <div className="text-center">
                ...
                <div className="font-medium capitalize">{t(`study.practice.generator.difficulty.${difficulty}`)}</div>
              </div>
              <div className="text-center">
                <div className="text-muted-foreground text-xs">{t('study.practice.generator.summary.estimatedTime')}</div>
                <div className="font-medium">{Math.ceil(questionCount * 1.5)}m</div>
              </div>
            </div>
          </div>
