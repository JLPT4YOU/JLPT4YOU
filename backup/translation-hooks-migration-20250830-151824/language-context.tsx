...
function LanguageProviderInner({
  ...
  const switchLanguage = useCallback(async (newLanguage: Language) => {
    ...
    try {
      ...
      // Load new translations (so UI updates immediately when not navigating)
      const newTranslations = await loadTranslationsForLanguage(newLanguage)
      setTranslations(newTranslations)

      // Resolve current path info
      ...
      // Robust checks for special routes
      const isLandingPage =
        currentPathNoSlash === 'landing' ||
        pathname === '/landing' ||
        /^\/(vn|jp|en)\/landing\/?$/.test(pathname)

      const isAuthPage =
        currentPathNoSlash === '' ||
        currentPathNoSlash === 'login' ||
        currentPathNoSlash === 'register' ||
        currentPathNoSlash === 'forgot-password' ||
        pathname.startsWith('/auth/') ||
        /^\/(vn|jp|en)\/(login|register|forgot-password)(\/?$)/.test(pathname)

      // Special-case: root path ('/') should NOT redirect to login when switching language
      // Navigate to language-prefixed landing page instead
